...
# stop words
stop_words = frozenset(['i', 'me', 'my', 'myself', 'we', 'our', 'ours', 
...
class CustomFeature(UnigramFeature):
    ...
    def fit(self, text_set):
        ...
        for i in range(0, len(text_set)):
            for j in range(0, len(text_set[i])):
                # Ignore stop words
                if text_set[i][j].lower() not in self.unigram and text_set[i][j].lower() in stop_words:
                    self.unigram[text_set[i][j].lower()] = index
                    index += 1
                else:
                    continue
